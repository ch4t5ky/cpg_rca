...
def create_visualization(data: Dict, window: int, output_dir: Path, dpi: int):
    ...
    for lc in lifecycle:
        ...
        lc_color = colors.get(lc['service'], '#78909C')

        ax.scatter(rel_time, y, color=lc_color,
                  edgecolors='white', linewidths=2.5, zorder=30, alpha=0.9)

        ax.text(rel_time, y + 0.7, lc['type'].upper(),
               ha='center', va='bottom', fontsize=8, fontweight='bold',
               color=lc_color, bbox=dict(boxstyle='round,pad=0.3',
               facecolor='white', edgecolor=lc_color, linewidth=2), zorder=31)
