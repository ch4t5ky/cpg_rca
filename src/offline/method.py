...
CFG EDGE CONDITIONS
───────────────────
Joern does NOT store TRUE/FALSE labels on CFG edges directly.
We derive branch conditions from AST structure:

  For each CONTROL_STRUCTURE with CONTROL_STRUCTURE_TYPE=IF:
    • AST child ORDER=1 → condition expression
      Its CFG successors are the branching points.
    • AST child ORDER=2 → THEN-block  (BLOCK node)
    • AST child ORDER=3 → ELSE-block  (BLOCK node, optional)

  The condition node (ORDER=1) typically has exactly two CFG-successors:
    • the successor reachable inside the THEN AST-subtree  → "TRUE"
    • the other successor                                  → "FALSE"

  We build a set of all AST-descendants of the THEN-block and check
  membership to assign the label.  For WHILE/FOR/DO the body block
  (ORDER=2 for WHILE/FOR, ORDER=1 for DO) is treated the same way.
"""
