...
Batch-обработчик готовых Code Property Graph (CPG) по сервисам.

Вход:
    Папка с сервисами, где для каждого сервиса лежит экспортированный
    CPG в формате DOT (Joern export), например:
...
Выход (для каждого сервиса создаётся output/<service>/...):
    - trie.png                        — визуализация Log Trie (лог-шаблоны из CPG)
    - <entrypoint>.flow.json          — FLOW: семантический граф вызовов от entrypoint'а
    - <entrypoint>.fsm.json           — FSM: конечный автомат переходов между лог-шаблонами
    - <entrypoint>.fsm.png (опц.)     — Graphviz-визуализация FSM
    - <entrypoint>.semantic.png (опц.)— Graphviz-визуализация FLOW
    - index.json                      — сводная таблица по всем entrypoint'ам сервиса
    - summary.json                    — сводка по сервису (кол-во entrypoints/templates/ошибок)

Использование:
    python builder.py --services-dir services --output-dir output
    python builder.py --services-dir services --output-dir output --service frontend
    python builder.py --services-dir services --output-dir output --render-graphviz

Ожидаемая структура проекта (см. github.com/ch4t5ky/cpg_rca):
    src/offline/log.py             -> build_templates_from_cpg, build_trie, visualize_trie_matplotlib
    src/offline/entrypoint.py      -> EntrypointDetector, Entrypoint
    src/offline/flow.py            -> EntrypointFlow, SemanticFlowGraph
    src/offline/finitestatemachine.py -> LogFlowExtractor
    src/offline/visual.py          -> draw_semantic_graph_graphviz, draw_log_fsm_graphviz
"""
